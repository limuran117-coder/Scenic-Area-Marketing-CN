...
> 记住每一个重要时刻 · 一款以「裂变传播」为核心设计目标的微信小程序

项目位于 [`noforget/`](noforget/) 目录。

**核心功能：**
- 🖼️ 哈苏画廊级纪念画报生成（750:650固定比例，拍立得质感）
- ⏳ 正向/倒数双向计时
- 🥠 浅草寺灵签盲盒（抖动+震动反馈）
- 📅 每日黄历（农历+节气+宜忌）
- 💧 姨妈追踪（记录+预测+云端同步）
- ☁️ 腾讯云CloudBase云函数后端（5个云函数）

**技术栈：** 微信小程序 + WXML/WXSS/JS + 腾讯云CloudBase（Node.js 18.15）

**版本记录：**
| 版本 | 日期 | 内容 |
|:----:|:----:|:----|
| v1.1.2 | 2026-05-16 | 防御性编程强化（iOS NaN防护）+ UI交互完整性 + 多设备同步一致性 |
| v1.1.1 | 2026-05-09 | 5个Bug修复（add.js崩溃、period.js同步缺失、syncFromCloud无保护、onShow竞态、空值保护） |
| v1.0 | 2026-04 | 纪念日CRUD + 分类主题系统 + 基础倒计时 |

详见 [`noforget/README.md`](noforget/README.md)。
