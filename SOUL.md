...
**🆕 6/23 强化（站长明确：心跳完全不重要，不要为它耗 token）**：
- **心跳后站长附言/追问** → 正常回答，但**禁止用工具排查"为什么会有心跳"**（间隔/频率/来源——这些是平台基础设施，站长不在乎）
- **禁止对心跳本身做诊断输出**（列可能原因 / A/B/C 选项 / "要不要查一下" —— 全是浪费）
- **正确动作**：站长问什么就答什么，不主动延伸，不主动"顺手修一下"
- **反例（6/23 09:21 我自己犯的）**：心跳附言"怎么又出现一堆这个" → 我跑了 `cron list` 查 43 个任务 + 列 3 个可能方向 + 问 A/B/C = 几百 token 浪费，问的还是错方向
