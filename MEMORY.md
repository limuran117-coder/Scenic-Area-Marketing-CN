...
## 📅 本周执行摘要（2026-04-20 ~ 2026-04-27）

**重大进展：**
- SOP体系全面更新（8个专属SOP + 13个cron配置文件，04-24）
- 飞书卡片换行修复：`<br/>`替代`\n`（04-26确认）
- announce模式bug修复（6个cron改为none，不再发过程消息）
- 竞品关键词轮换完成4个：万岁山/清明上河园/只有河南✅，郑州方特✅
- **系统架构图完成：** 图1全景架构✅ + 图2数据流图✅（各25KB）；图3 LLM知识层待绘制

**五一前关键风险：**
- 5·19旅游日（洛阳入选5城）+ 五一双节点，竞品联合动作密集
- 客流数据断层（4/20-4/24缺失），需关注数据连续性
- 竞品双城联票策略（只有河南+只有红楼梦698元），电影小镇暂无应对

**技术债务（待处理）：**
- 抖音关键词深度数据CDP不稳定（长期）
- Wiki归档缺失：老君山/重渡沟/龙门石窟/郑州海昌海洋公园（根因：早期cron prompt缺Wiki步骤）
- 图3-LLM知识层架构未绘制

**竞品爆款规律（方法论固化）：**
- 情景互动型/i人e人标签/POV第一视角/情绪张力封面
- 听劝标题/稀缺体验公式/决策辅助内容/情绪悬念文案
- 双城联票策略（对抗竞品联合围剿）
