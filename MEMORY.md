...
**【7/25】建业电影小镇 → 郑州电影小镇（7/25 10:21 站长宣布）**
**Why:** 集团品牌策略调整；旧名已不准确
**How:**
- **采集关键词双轨**：旧名「建业电影小镇」+ 新名「郑州电影小镇」同步采集，避免更名过渡期历史指数断档
- **新名优先**：日报/卡片/对外文案默认用「郑州电影小镇」；旧名只在引用历史数据时出现
- **脚本默认关键词**：`douyin_index.py` / `xiaohongshu_crawl.py` 默认搜索词已切到「郑州电影小镇」（7/25 执行）
- **小红书灵犀后台**：账号名是否同步改 = 站长待确认（暂不动，等站长答复）
- **历史数据**：(N).csv / 历年 Excel / wiki SOP 等历史档案保留旧名（事实记录），不动
